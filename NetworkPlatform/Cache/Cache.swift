...
final class Cache<T: DomainPlatform.Encodable>: AbstractCache where T == T.Encoder.DomainType {
    ...
    private let cacheScheduler = SerialDispatchQueueScheduler(internalSerialQueueName: "com.mobdev125.Network.Cache.queue")
    ...
    func save(object: T) -> Completable {
        return Completable.create { (observer) -> Disposable in
            ...
            if #available(iOS 11.0, *) {
                do {
                    try NSKeyedArchiver.archivedData(withRootObject: object.encoder, requiringSecureCoding: false)
                        .write(to: path)
                    observer(.completed)
                } catch {
                ...
                if NSKeyedArchiver.archiveRootObject(object.encoder, toFile: path.absoluteString) {
                    observer(.completed)
                } else {
    ...
    func save(objects: [T]) -> Completable {
        return Completable.create { (observer) -> Disposable in
            ...
            do {
                if #available(iOS 11.0, *) {
                    try NSKeyedArchiver.archivedData(withRootObject: objects.map{ $0.encoder }, requiringSecureCoding: false)
                        .write(to: path)
                } else {
    ...
    func fetch(withID id: String) -> Maybe<T> {
        return Maybe<T>.create { (observer) -> Disposable in
            ...
            guard let object = NSKeyedUnarchiver.unarchiveObject(withFile: path.absoluteString) as? T.Encoder else {
                observer(.completed)
                return Disposables.create()
            }
